...
public class ServerFileSend {
    ...
    public void startServer() throws IOException {
        ...
        JFrame jFrame = new JFrame("Nhận File");
        // Set the size of the frame.
        jFrame.setSize(400, 400);
        // Give the frame a box layout that stacks its children on top of each other.
        jFrame.setLayout(new BoxLayout(jFrame.getContentPane(), BoxLayout.Y_AXIS));

        GraphicsConfiguration config = jFrame.getGraphicsConfiguration();
        Rectangle bounds = config.getBounds();
        Insets insets = Toolkit.getDefaultToolkit().getScreenInsets(config);

        int x = bounds.x + bounds.width - insets.right - jFrame.getWidth();
        ...
        // Title above panel.
        JLabel jlTitle = new JLabel("Menu nhận file");
    ...
    public static JFrame createFrame(final String fileName, final byte[] fileData, String fileExtension) {
        ...
        JLabel jlPrompt = new JLabel("Xác nhận tải file về máy ? " + fileName + "?");
